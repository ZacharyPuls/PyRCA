...
from ...score.LocalScoreFunctionClass import LocalScoreClass
...
from ...utils.PCUtils import Meek
from ...utils.PDAG2DAG import pdag2dag
from ...utils.BackgroundKnowledge import BackgroundKnowledge


def ges(X: ndarray, score_func: str = 'local_score_BIC', maxP: Optional[float] = None, background_knowledge: BackgroundKnowledge = None,
        parameters: Optional[Dict[str, Any]] = None, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    # G = pdag2dag(G)
    # G = dag2cpdag(G)
    cG = CausalGraph(G.num_vars, G.get_node_names())
    cG.G = G
    G = Meek.meek(cG, background_knowledge=background_knowledge).G
    ...
    while True:
        ...
        for i in range(N):
            for j in range(N):
                if background_knowledge is not None and background_knowledge.is_forbidden(G.nodes[i], G.nodes[j]):
                    continue
                ...
                if (G.graph[i, j] == Endpoint.NULL.value and G.graph[j, i] == Endpoint.NULL.value
                    ...
                    for k in range(len(sub)):
                        ...
                        if background_knowledge is not None:
                            T = sub[k]
                            for l in range(len(T)):  # check every edge between T and Xj as T->Xj
                                if background_knowledge.is_forbidden(G.nodes[T[l]], G.nodes[j]):
                                    # if T->Xj is forbideen
                        ...
                        if (S[k] < 2):  # S indicate whether we need to check subset(k)
                            ...
                            if (V1):
                                if (not S[k]):
                                    ...
                                    V2 = 1
                                if (not violates_knowledge and V2):
        ...
        if (len(min_desc) != 0):
            ...
            if (score - score_new <= 0):
                break
            G = insert(G, min_desc[0], min_desc[1], min_desc[2], verbose)
    ...
    while True:
        ...
        for i in range(N):
            for j in range(N):
                if background_knowledge is not None and not background_knowledge.noedge_required(G.nodes[i], G.nodes[j]):
                    continue
                ...
                if ((G.graph[j, i] == Endpoint.TAIL.value and G.graph[i, j] == Endpoint.TAIL.value)
                    ...
                    for k in range(len(sub)):
                        ...
                        if background_knowledge is not None:
                            T = sub[k]
                            for l in range(len(T)):  # check every edge  Xj->T, Xi->T
                                if background_knowledge.is_forbidden(G.nodes[j], G.nodes[T[l]]) \
                                        or background_knowledge.is_forbidden(G.nodes[i], G.nodes[T[l]]):
                                    # if edge is forbideen
                                    violates_knowledge = True
                        if (S[k] == 1):
                        ...
                        if (not violates_knowledge and V):
        ...
        if len(min_desc) != 0:
            ...
            if score - score_new <= 0:
                break
            G = delete(G, min_desc[0], min_desc[1], min_desc[2], verbose)
