...
from ..utils.BackgroundKnowledge import BackgroundKnowledge
...
def score_g(Data, G, score_func, parameters, background_knowledge):  # calculate the score for the current G
    # here G is a DAG
    score = 0
    for i, node in enumerate(G.get_nodes()):
        PA = G.get_parents(node)
        if background_knowledge is not None:
            for parent_node in PA:
                if background_knowledge.is_forbidden(parent_node, node):
                    PA.remove(parent_node)
...
def insert(G, i, j, T, verbose):
    # Insert operator
    # insert the directed edge Xi->Xj
    nodes = G.get_nodes()
    G.add_edge(Edge(nodes[i], nodes[j], Endpoint.TAIL, Endpoint.ARROW))
    if verbose:
        print("Insert edge by forward step: " + str(G.get_edge(nodes[i], nodes[j])))
    ...
    for k in range(len(T)):  # directing the previous undirected edge between T and Xj as T->Xj
        ...
        if verbose:
            print("Directing edge by forward step: " + str(G.get_edge(nodes[T[k]], nodes[j])))
...
def delete(G, i, j, H, verbose):
    # Delete operator
    nodes = G.get_nodes()
    if G.get_edge(nodes[i], nodes[j]) is not None:
        # delete the edge between Xi and Xj
        if verbose:
            print("Delete edge by backward step: " + str(G.get_edge(nodes[i], nodes[j])))
    ...
    for k in range(len(H)):  # directing the previous undirected edge
        ...
        if verbose:
            print("Directing edge by backward step: " + str(G.get_edge(nodes[j], nodes[H[k]])))
            print("Directing edge by backward step: " + str(G.get_edge(nodes[i], nodes[H[k]])))
    return G
...
def add_required_edges(G, background_knowledge, verbose=False):
    ...
    for nodeA, nodeB in background_knowledge.get_all_required_by_node():
        if(not G.is_ancestor_of(nodeB, nodeA)):
            if G.get_edge(nodeA, nodeB) is not None:
                G.remove_edge(G.get_edge(nodeA, nodeB))
            G.add_edge(Edge(nodeA, nodeB, Endpoint.TAIL, Endpoint.ARROW))
        if verbose:
            print("Adding edge by knowledge: " + str(G.get_edge(nodeA, nodeB)))

    for edge in G.get_graph_edges():
        nodeA = edge.get_node1()
        nodeB = edge.get_node2()

        if background_knowledge.is_forbidden(nodeA, nodeB):
            if  G.is_adjacent_to(nodeA, nodeB) and not G.is_child_of(nodeA, nodeB):
                if (not G.is_ancestor_of(nodeA, nodeB)):
                    if G.get_edge(nodeA, nodeB) is not None:
                        G.remove_edge(G.get_edge(nodeA, nodeB))
                    G.add_edge(Edge(nodeB, nodeA, Endpoint.TAIL, Endpoint.ARROW))
                    if verbose:
                        print("Adding edge by knowledge: " + str(G.get_edge(nodeB, nodeA)))
            if not G.is_child_of(nodeA, nodeB) and background_knowledge.is_forbidden(nodeA, nodeB):
                if (not G.is_ancestor_of(nodeA, nodeB)):
                    if G.get_edge(nodeA, nodeB) is not None:
                        G.remove_edge(G.get_edge(nodeA, nodeB))
                    G.add_edge(Edge(nodeB, nodeA, Endpoint.TAIL, Endpoint.ARROW))
                    if verbose:
                        print("Adding edge by knowledge: " + G.get_edge(nodeB, nodeA))
        elif background_knowledge.is_forbidden(nodeB, nodeA):
            nodeA = edge.get_node2()
            nodeB = edge.get_node1()
            if  G.is_adjacent_to(nodeA, nodeB) and not G.is_child_of(nodeA, nodeB):
                if (not G.is_ancestor_of(nodeA, nodeB)):
                    if G.get_edge(nodeA, nodeB) is not None:
                        G.remove_edge(G.get_edge(nodeA, nodeB))
